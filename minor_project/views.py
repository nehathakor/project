...
def signup(request):
    if request.method == 'POST':
        ...
        mongo_models.create_user(fullname, username, email, password)
...
def login(request):
    if request.method == 'POST':
        ...
        user = mongo_models.get_user_by_username(username)

        if user and user['password'] == password:
            # Authentication successful, redirect to profile or dashboard
            return redirect('index')
        else:
